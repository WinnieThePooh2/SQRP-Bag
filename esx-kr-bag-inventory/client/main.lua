...
function Bag()
    ...
    local elements = {}
        ...
        table.insert(elements, {label = 'Lägg in objekt', value = 'put'})
        table.insert(elements, {label = 'Ta ut objekt', value = 'take'})
        table.insert(elements, {label = 'Lägg ner väskan', value = 'drop'})
        ...
        local wait = 500
            ...
            for i=1, #Bags, 1 do
                ...
                if GetDistanceBetweenCoords(playercoords, Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z, true) <= 1.5 and not HasBag then
                    ...
                    wait = 5
                    Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.45, '~g~[E]~w~För att ta upp väskan ')
...
Citizen.CreateThread(function()
    while true do
        Wait(5)
        if IsControlJustReleased(0, Keys['F10']) and HasBag and not IsPedInAnyVehicle(GetPlayerPed(-1), true) and not IsEntityInAir(PlayerPedId()) then -- Change F5 to the key you want to open the meny with f10
            Bag()
        end
    end
end)
