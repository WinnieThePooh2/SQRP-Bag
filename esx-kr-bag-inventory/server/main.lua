...
ESX.RegisterUsableItem('bag', function(source)
    ...
    if not HasBag then
        ...
        TriggerClientEvent('esx:showNotification', source, '~r~Du har redan en väska på dig.')
    end
end)
...
AddEventHandler('esx-kr-bag:PutItem', function(id, item, label, count, type)
    ...
    elseif type == 'item' then
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_bag_inventory WHERE id = @id ',{["@id"] = id}, function(result)
            ...
            TriggerClientEvent('esx:showNotification', src, '~r~Du kan bara ha ' .. Config.MaxItemCount .. ' olika föremål i din väska')
        end
    end)
end)
...
AddEventHandler('esx-kr-bag:PickUpBag', function(id)
	...
	TriggerClientEvent('esx:showNotification', src, 'Tryck ~b~[F10]~w~ för att komma åt din väska.')
        TriggerClientEvent('esx-kr-bag:ReSync', xPlayers[i], id)
     end
end)
